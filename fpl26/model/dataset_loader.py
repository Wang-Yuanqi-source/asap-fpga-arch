...
from arch_preprocessing import xml_to_dgl_graph
...
@dataclass
class Sample:
    g: dgl.DGLGraph                        # 电路 AAG 图
    arch_g: dgl.DGLGraph                   # 架构 XML 图
...
class CircuitSeqDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        # decide columns
        if use_header:
            if seq_col is None:     seq_col = df.columns[0]   # 这里仍沿用原来的参数名，但语义已经是 arch_path
        ...
        self.seq_col = seq_col  # 列名仍沿用 seq_col，但其含义为 arch_path
        ...
        df_ok[self.seq_col] = df_ok[self.seq_col].astype(str).map(
            lambda p: _resolve_path(self.arch_dir, p)
        )
        self.df = df_ok
    ...
    def _load_arch_graph(self, arch_path: str) -> dgl.DGLGraph:
        """
        解析架构 XML，返回 DGLGraph。
        这里简单做一个基于路径的缓存，避免同一个架构被重复解析多次。
        """
        if not hasattr(self, "_arch_cache"):
            self._arch_cache: Dict[str, dgl.DGLGraph] = {}

        arch_path = str(arch_path)
        if arch_path in self._arch_cache:
            return self._arch_cache[arch_path]

        g_arch, _ = xml_to_dgl_graph(arch_path, label=0.0, node_labels={})
        self._arch_cache[arch_path] = g_arch
        return g_arch
    ...
    def __getitem__(self, idx: int) -> Sample:
        ...
        gid = int(row["gid"])
        arch_path = str(row[self.seq_col])

        g = self._get_graph(gid)
        arch_g = self._load_arch_graph(arch_path)
        ...
        return Sample(g=g, arch_g=arch_g, y=y, y_cls=None)
...
def collate_circuit_seq(samples: List[Sample]) -> Dict[str, Any]:
    ...
    graphs_circ = [s.g for s in samples]
    graphs_arch = [s.arch_g for s in samples]

    bg_circ = dgl.batch(graphs_circ)
    bg_arch = dgl.batch(graphs_arch)
